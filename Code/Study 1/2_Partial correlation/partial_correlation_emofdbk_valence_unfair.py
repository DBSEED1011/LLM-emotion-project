...
participant_df = trial_df.groupby(['participant_id', 'group', 'cost_level', 'fairness_group']).agg({
    ...
    'EmoFDBK_valence': 'mean',  # EmoFDBK_valence_mean
    'EmoFDBK_arousal': 'mean',  # EmoFDBK_arousal_mean (for consistency, though not used in this analysis)
...
for condition in conditions:
    ...
    plt.tight_layout()
    filename = f'partial_correlation_emofdbk_valence_{condition}.png'
...
print(f"\n=== Key Notes ===")
print("1. Use trial level data and aggregate to participant level choice mean and EmoFDBK_valence mean")
