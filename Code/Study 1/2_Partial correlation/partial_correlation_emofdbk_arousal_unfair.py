...
# Read trial level data and aggregate to participant level
"""
DATA FILE NOTE:
...
"""
print("=== Reading trial level data and aggregating to participant level ===")
# Read data - Using demo data - see DATA FILE NOTE above for full data access
trial_df = pd.read_excel('demo.xlsx')

# Create participant_id (use id directly, matching original format)
trial_df['participant_id'] = trial_df['id']

# Determine fairness_group based on amount_of_allocation
# amount_of_allocation = 15 corresponds to fair (15:15), others are unfair
trial_df['fairness_group'] = trial_df['amount_of_allocation'].apply(
    lambda x: 'fair' if x == 15 else 'unfair'
)

# Aggregate to participant level: group by participant_id, group, cost_level, fairness_group
print("Aggregating trial level data to participant level...")
participant_df = trial_df.groupby(['participant_id', 'group', 'cost_level', 'fairness_group']).agg({
    'choice': 'mean',  # choice_mean
    'EmoFDBK_valence': 'mean',  # EmoFDBK_valence_mean (for consistency, though not used in this analysis)
    'EmoFDBK_arousal': 'mean',  # EmoFDBK_arousal_mean
...
participant_df.rename(columns={
    'choice': 'choice_mean',
    'EmoFDBK_valence': 'EmoFDBK_valence_mean',
    'EmoFDBK_arousal': 'EmoFDBK_arousal_mean',
    'trial': 'n_trials'
}, inplace=True)

print(f"Trial level data: {len(trial_df)} trials")
print(f"Participant level data: {len(participant_df)} participant-condition combinations")
print(f"Total number of participants: {len(participant_df['participant_id'].unique())}\n")

print("=== Partial Correlation Analysis: Choice Mean vs EmoFDBK_arousal Mean (Reference unfair format) ===")
...
for condition in conditions:
    ...
    for group_name in condition_data['group'].unique():
        ...
        y = group_data['EmoFDBK_arousal_mean'].values  # Use EmoFDBK_arousal mean as y-axis
        ...
        if not np.isnan(partial_r):
            ...
            jitter_amount = 5.0  # Jitter amount (adjusted for arousal values)
    ...
    ax.set_ylabel('Emotional outcome arousal', fontsize=7)
    ...
    plt.tight_layout()
    filename = f'partial_correlation_emofdbk_arousal_{condition}.png'
...
# Save results to CSV
results_df.to_csv('partial_correlation_emofdbk_arousal_results.csv', index=False)
print("\nResults saved to: partial_correlation_emofdbk_arousal_results.csv")

print(f"\n=== Key Notes ===")
print("1. Use trial level data and aggregate to participant level choice mean and EmoFDBK_arousal mean")
...
print("6. Charts saved separately as: partial_correlation_emofdbk_arousal_unfair_format.png and partial_correlation_emofdbk_arousal_fair_format.png")
