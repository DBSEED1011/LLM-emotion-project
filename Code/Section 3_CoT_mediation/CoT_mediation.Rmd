...
# 🧠 Define keywords related to unfairness
unfair_keywords <- c("unfair", "unfairness", "unequal", "injustice", "biased", "not fair")

# ✂️ Convert text to lowercase for keyword matching
text_lower <- tolower(df_with_emotions$Reasoning)

# ✅ Count how many times unfairness-related keywords appear in each text (as a score)
df_with_emotions$unfair_score <- str_count(text_lower, str_c(unfair_keywords, collapse = "|"))

# ✅ Optionally, add a Boolean column (whether it contains at least one unfairness keyword)
...
# 1️⃣ Count the number of punishment choices
...
# 2️⃣ Calculate the mean (or median) of emotion variables for each group
...
if (is.numeric(df$choice)) {
  # If it’s not 0/1 but has only two distinct numeric values, map them to 0/1
  ...
  if (length(lev) != 2) {
    stop(paste("‘choice’ is not a binary variable (detected levels:", length(lev), "). Please check your data."))
  }
...
df$choice <- as.numeric(df$choice)
df$choice_next <- as.ordered(df$choice_next)  # If it’s a binary choice
