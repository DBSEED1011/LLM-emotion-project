...
# Try to get the directory of the current Rmd file, fallback to working directory
if (requireNamespace("rstudioapi", quietly = TRUE) && rstudioapi::isAvailable()) {
  script_dir <- dirname(rstudioapi::getActiveDocumentContext()$path)
} else {
  script_dir <- getwd()
}
file_path <- file.path(script_dir, "Study2a_experimental_data_emo_vs_no.xlsx")
...
# Generally contains columns: term, estimate, std.error, statistic, p.value, conf.low, conf.high

# Combine results table: logit coefficients + CI + OR
results_m1 <- coefs_m1 %>%
  mutate(
    OR      = exp(estimate),
    OR_low  = exp(conf.low),
...
# Export to CSV (change to your desired path)
write.csv(
  results_m1,
...
## ----- Estimate marginal means (for pairwise only, default is logit scale) -----
emm_logit <- emmeans(m1, ~ Agent_type * Emotion_selfreport)

## 1) Compare different Agent_type within each Emotion_selfreport condition
posthoc_agent <- pairs(
  emm_logit,
  by = "Emotion_selfreport"   # Compare Agent within each Emotion condition
) %>%
  summary(infer = TRUE) %>%    # infer = TRUE provides 95% CI and z, p
  as.data.frame() %>%
  mutate(effect = "Agent_type within Emotion_selfreport")

## 2) Compare Emotion_selfreport (Yes vs No) within each Agent_type
posthoc_emotion <- pairs(
  emm_logit,
  by = "Agent_type"           # Compare Emotion within each Agent
) %>%
  summary(infer = TRUE) %>%
  as.data.frame() %>%
  mutate(effect = "Emotion_selfreport within Agent_type")

## 3) Combine & standardize column names
posthoc_all <- bind_rows(posthoc_agent, posthoc_emotion)

results_posthoc_m1 <- posthoc_all %>%
  rename(
    B        = estimate,   # log-odds difference
    SE       = SE,
    z        = z.ratio,
    P        = p.value,
    CI_lower = asymp.LCL,  # logit 95% CI lower bound
    CI_upper = asymp.UCL   # logit 95% CI upper bound
  ) %>%
  mutate(
    OR       = exp(B),
    OR_low   = exp(CI_lower),
  ...
  # Keep key information columns
  select(
    Emotion_selfreport,   # Or you may prefer to put it before contrast
    Agent_type,
    contrast,
    effect,
...
# GLMM4
# ============ Read Data ============
# Use the same file path as above (sheet 2 for GLMM4)
file_path <- file.path(script_dir, "Study2a_experimental_data_emo_vs_no.xlsx")
