...
```{r}
# Report 95% CI (logit scale)
coefs <- summary(m1)$coefficients

# Compute 95% Wald CI on logit scale
coefs_CI <- cbind(
  B        = coefs[, "Estimate"],
  SE       = coefs[, "Std. Error"],
  z        = coefs[, "z value"],
...
```{r}
# (1) Compute conditional means for each Agent_type × Emotion_selfreport combination
emm_tab <- emmeans(m1, ~ Emotion_selfreport | Agent_type, type = "response")

# (2) Conduct pairwise comparisons (Yes vs. No) with CI and p-values
ci_tab <- summary(
  pairs(emm_tab, reverse = TRUE),
  infer = c(TRUE, TRUE)
)

print(ci_tab)
# Columns include odds.ratio, lower.CL, upper.CL, and p.value
...
```{r}
# ============ Extended Model with Higher-Order Interactions ============
expdata_complete$Agent_type <- factor(expdata_complete$Agent_type)
expdata_complete$Agent_type <- relevel(expdata_complete$Agent_type, ref = "Humans")
...
cat("\n===== Model 3 (Extended Interaction Model) Results =====\n")
print(summary(m2))
